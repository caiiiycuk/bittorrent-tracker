...
class Swarm {
  ...
  _getPeers (numwant, ownPeerId, isWebRTC) {
    ...
    const disabledSeedersEntry = seedFilter && isWebRTC && server._disabledSeedersMap
      ? server._disabledSeedersMap[this.infoHash]
      : null
    const disabledPeerIds = disabledSeedersEntry ? disabledSeedersEntry.peersSet : null
    ...
    while ((peerId = ite())) {
      ...
      if (disabledPeerIds && disabledPeerIds.has(peer.peerId)) continue
    ...
    if (disabledPeerIds) {
      debug('_getPeers disabled filter: infoHash=%s disabled=%d returned=%d numwant=%d',
        this.infoHash.substring(0, 8), disabledPeerIds.size, peers.length, numwant)
    }
